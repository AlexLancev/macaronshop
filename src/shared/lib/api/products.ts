...
function transformSupabaseProductToProductCard(
	...
	return {
		...
		productDetails: prodDetailsData[typeProduct === 'macaron' ? 'macaron' : 
			...
			typeProduct === 'waferRolls' ? 'waferRolls' : 'potato'],
	} as ProductCardType;
}
...
export async function getAllProductsByHoliday(
	...
	try {
		...
		const productTypes: ProductType[] = ['macaron', 'eclairs', 'waferRolls', 'potato'];
			...
			for (const rows of allRows) {
				...
				for (const [columnName, columnValue] of Object.entries(rows)) {
					...
					if (productData && productData.typeHoliday === typeHoliday) {
						...
						const transformedProduct = transformSupabaseProductToProductCard(productData, typeProduct);
